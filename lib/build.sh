...
resolve_node() {
  ...
  local node_file=$(curl --silent --get --retry 5 --retry-max-time 15 $lookup_url -f | grep -oE  'node-v[0-9]+\.[0-9]+\.[0-9]+-linux-x64\.tar\.gz' | head -n1)
  if [ "$?" -eq "0" ]; then
...
download_node() {
  ...
  if [ ! -f ${cached_node} ]; then
    ...
    echo "DEBUG - url: $url"
    echo "DEBUG - cached_node: $cached_node"
